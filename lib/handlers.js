...
/**
 * Sends a request to the checks sub-methods to perform
 * a monitor function a given url data
 */
...
/**
 * Container object for checks service
 */
handlers._checks = {};

/**
 * Sends a POST request to a resource on a url, returns
 * the server response and the request object
 * 
...
handlers._checks.post = function (data, callback) {
    ...
    var timeout = typeof(data.payload.timeout) === 'number' && data.payload.timeout % 1 === 0 && data.payload.timeout >= 1 ? data.payload.timeout : 5;

    if (protocol && url && method && successCodes && timeout) {
        // get the token from the headers
        var token = typeof(data.headers.token) === 'string' && data.headers.token ? data.headers.token : false;
        // look up the user data by reading the token
        _data.read('tokens', token, function (err, tokenData) {
            if (!err && tokenData) {
                ...
                _data.read('users', userPhone, function(err, userData) {
                    if (!err && userData) {
                        ...
                        if (userChecks.length < config.maxChecks) {
                            // assign id to each checks
                            var checkId = helpers.generateRandomString(22);
                                ...
                                if (!err) {
                                    ...
                                    _data.update('users', userPhone, userData, function (err) {
                                        if (!err) {
                                            ...
                                            callback(500, {'Error':'could not updatethe user with the new check'});
                                        }
                                    });
                                } else {
                            ...
                            callback(400, {'Error':'User\'s maxed out allocated checks. Hence, peek limit'});
                        }
                    } else {
                        callback(403);
                    }
                ...
                callback(403, {'Error':'Unauthorized request. Provide a security token'});
            }
        });
    } else {
        callback(400, {'Error':'Missing required paarameters'});
    }
};

/**
 * Checks if a resource on a given url is available and 'online'
 * 
 * Required: 
 * 
 * @param {*} data 
...
/**
 * Sends a mderated request to a server resource,
 * in another sense a post function with tweaked params
 * 
 * Required data: id
 * 
 * Optional: protocol, url, method, successCodes, timeout (at least one of these variables must be provided)
 * @param {*} data 
 * @param {[ststusCode, response.data]} callback 
 */
handlers._checks.put = function (data, callback) {};

handlers._checks.delete = function (data, callback) {
    // check the check id vaildity
